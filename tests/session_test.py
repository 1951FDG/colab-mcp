...
class TestColabProxyMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_on_call_tool_injected_returns_as_is(self, mock_proxy_client):
        middleware = session.ColabProxyMiddleware(mock_proxy_client)
        context = Mock()
    ...
    @pytest.mark.asyncio
    async def test_on_call_tool_colab_tool_compresses(self, mock_proxy_client):
        middleware = session.ColabProxyMiddleware(mock_proxy_client)
        context = Mock()
        context.message.name = "run_code_cell"
        call_next = AsyncMock()
        call_next.return_value = ToolResult(content=[TextContent(type="text", text="\n".join(str(i) for i in range(200)))])

        result = await middleware.on_call_tool(context, call_next)

        # Long output should be compressed
...
class TestCheckSessionProxyToolFn:
    ...
    async def test_connected(self):
        ...
        assert await session.check_session_proxy_tool_fn(ctx) == "Already connected"

    @pytest.mark.asyncio
    async def test_disconnected_no_create_new(self):
        ctx = Mock()
        ...
        result = await session.check_session_proxy_tool_fn(ctx)
        assert result == "mcpProxyToken=test-token&mcpProxyPort=1234"

    @pytest.mark.asyncio
    async def test_disconnected_create_new(self, mock_webbrowser):
        ctx = Mock()
...
class TestCompressToolResult:
    def test_short_output_not_compressed(self):
        ...
        compressed = session._compress_tool_result(result)
        assert len(compressed.content[0].text.split("\n")) == 50

    def test_long_output_compressed(self):
        text = "\n".join(str(i) for i in range(200))
        result = ToolResult(content=[TextContent(type="text", text=text)])
        compressed = session._compress_tool_result(result)
        lines = compressed.content[0].text.split("\n")
        # 50 head + 1 info line + 50 tail = 101
        assert len(lines) == 101
        assert lines[0] == "0"
        assert lines[49] == "49"
        assert "100 lines omitted" in lines[50]
        assert lines[100] == "199"

    def test_exactly_max_lines_not_compressed(self):
    ...
    def test_non_text_content_preserved(self):
        ...
        compressed = session._compress_tool_result(result)
        assert compressed.content[0].text == "short"

    def test_structured_content_compressed(self):
        """structured_content text arrays should be compressed for run_code_cell output format."""
        lines = [f"[{i:05d}] test line\n" for i in range(200)]
        structured = {"outputs": [{"output_type": "stream", "name": "stdout", "text": lines}]}
        result = ToolResult(
            content=[TextContent(type="text", text="full")],
            structured_content=structured,
        )
        compressed = session._compress_tool_result(result)
        assert compressed.structured_content is not None
        compressed_text = compressed.structured_content["outputs"][0]["text"][0]
        assert "lines omitted" in compressed_text
        # Verify head and tail are present
        assert "[00000] test line" in compressed_text
        assert "[00199] test line" in compressed_text

    def test_structured_content_short_preserved(self):
        """Short structured_content text arrays should not be compressed."""
        lines = [f"line {i}\n" for i in range(10)]
        structured = {"outputs": [{"output_type": "stream", "name": "stdout", "text": lines}]}
        result = ToolResult(
            content=[TextContent(type="text", text="full")],
        ...
        compressed = session._compress_tool_result(result)
        # text array should be unchanged (10 < 100)
        assert len(compressed.structured_content["outputs"][0]["text"]) == 10
